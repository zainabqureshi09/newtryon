...
export default function Home() {
    ...
    fetch("http://localhost:1337/api/products?populate=*",
      ...
      .then(json => {
        const mapped: Product[] = json.data.map((p: any) => {
          ...
          const overlayUrl =
            attrs?.overlayImage?.data?.attributes?.url || attrs?.overlayImage?.url || null;
          ...
          return {
            ...
            overlayImage: overlayUrl ? "http://localhost:1337" + overlayUrl : null,
          };
        });
        setProducts(mapped);
      })
  ...
  return (
    <div className="products-page grid grid-cols-1 md:grid-cols-3 gap-6">
      {products.map((p) => (
        <div key={p.id} className="product-item border p-4 rounded-xl shadow-md bg-white">
          ...
          {p.overlayImage && (
            <>
              <button
                ...
                onClick={() => setActiveTryOn(activeTryOn === p.id ? null : p.id)}
              >
                {activeTryOn === p.id ? "❌ Close Try-On" : "👓 Try On"}
              </button>

              {activeTryOn === p.id && (
                <TryOn glassesUrl={p.overlayImage} />
              )}
            </>
          )}
        </div>
...
// ✅ TryOn component
function TryOn({ glassesUrl }: { glassesUrl: string }) {
